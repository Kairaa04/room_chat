...
const Try = () => {
  ...
  useEffect(() => {
    ...
    const savedDownloadedVideos = localStorage.getItem("downloadedVideos");

    // Load downloaded videos from localStorage if available
    if (savedDownloadedVideos) {
      setDownloadedVideos(JSON.parse(savedDownloadedVideos));
    }

    if (lastDownloadDate === today && savedDownloads) {
  ...
  // Save to localStorage when downloadsToday, isPremium, or downloadedVideos changes
  useEffect(() => {
  ...
  }, [downloadsToday, isPremium, downloadedVideos]);
  ...
  const handleDownload = () => {
    ...
    // Update the download count and store the current date
    const newDownloadCount = downloadsToday + 1;
    setDownloadsToday(newDownloadCount);
    localStorage.setItem("downloadsToday", newDownloadCount);
    localStorage.setItem("lastDownloadDate", new Date().toDateString());

    // Add the downloaded video to the user profile
    setDownloadedVideos((prev) => [
      ...prev,
      { name: "video.mp4", date: new Date().toDateString() },
    ]);
    ...
    >
      ...
      <div style={{ marginTop: "20px" }}>
        <button
          ...
          disabled={downloadsToday >= dailyDownloadLimit && !isPremium}
        >
          Download Video
        </button>
        ...
        <div style={{ marginTop: "40px" }}>
          <h2 style={{ color: isSouthIndia ? "#000" : "#fff" }}>
            Your Downloads
          </h2>
          {downloadedVideos.length > 0 ? (
            <ul style={{ color: isSouthIndia ? "#000" : "#fff" }}>
              {downloadedVideos.map((video, index) => (
                <li key={index}>
                  {video.name} - Downloaded on {video.date}
                </li>
              ))}
            </ul>
          ) : (
